...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
        return playAdventurer(drawntreasure, state, temphand, currentPlayer);
			
    case council_room:
        return playCouncil_room(currentPlayer, state, handPos);
			
    case feast:
        return playFeast(currentPlayer, state, temphand, choice1);
...
int playAdventurer(int drawntreasure, struct gameState *state, int temphand[], int currentPlayer)
{
  int cardDrawn;
  int z = 0;
